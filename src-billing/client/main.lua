...
RegisterNetEvent("src-billing:input", function(data)
    local input = lib.inputDialog("LASKUT", {
        { type = "number", label = "Summa", description = "Valitse laskun määrä.", min = 1, required = true, icon = "dollar-sign" },
        { type = "input", label = "Syy", description = "Laskun syy.", required = true, icon = "arrow-right" }
    })
...
RegisterNetEvent("src-billing:mybills:menu", function()
    lib.registerContext({
        ...
        title = "Laskut",
        options = {
            { title = "Maksetut Laskut", description = "Laskut jotka olet jo maksanut.", icon = "file-invoice-dollar", event = "src-billing:mybills:paid" },
            { title = "Maksamattomat Laskut", description = "Laskut joita et ole vielä maksanut.", icon = "file-invoice-dollar", event = "src-billing:mybills:unpaid" }
        }
    })
    ...
    for i, job in ipairs(Config.Jobs) do
        if job == PlayerData.job.name then
            lib.registerContext({
                ...
                title = "Laskut",
                options = {
                    { title = "Minun Laskut", description = "Omat laskusi.", icon = "file-invoice-dollar", event = "src-billing:mybills:menu" },
                    { title = "Firman Laskut", description = "Tarkista Firman laskut.", icon = "file-invoice-dollar", event = "src-billing:society" },
                    { title = "Lähetä Lasku", description = "Lähetä lasku pelaajalle.", icon = "file-invoice-dollar", event = "src-billing:send" }
                }
            })
